...
#Region "Main form control methods"
    ...
    Private Sub frmMain_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Dim sJson As String = "{""s"":""1"",""y"":""2"" }"
        Dim jsonData As Dictionary(Of String, Object) = CType(serializer.DeserializeObject(sJson), Dictionary(Of String, Object))

        Debug.Print(jsonData.Item("s"))



        'endof json test
        Me.Text = "Nuri ConfigTool (ver" & Application.ProductVersion & ")"
        Dim b As Int16 = -9
        ...
        'add combobox data
        With cboBaud
            .Items.Add(New myDataItem("1200", 0))
            .Items.Add(New myDataItem("2400", 1))
            .Items.Add(New myDataItem("4800", 2))
            .Items.Add(New myDataItem("9600", 3))
            .Items.Add(New myDataItem("19200", 4))
            .Items.Add(New myDataItem("38400", 5))
            .Items.Add(New myDataItem("57600", 6))
            .Items.Add(New myDataItem("115200", 7))
        End With
        ...
        With cboFlow
            .Items.Add(New myDataItem("NONE", 0))
            .Items.Add(New myDataItem("CTS/RTS", 1))
        End With
        'tree view
        ...
        'disable txtpwd in order to handle a new pwd is input or not
    ...
    Private Sub tsbSet_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbSet.Click
        ...
        ElseIf rbDHCP.Checked Then
            sec.IpMode = 1
        End If
        'mode: client, server, mixed, udp
        If rbTcpClient.Checked Then sec.Mode = 0
        If rbTcpServer.Checked Then sec.Mode = 1
        If rbTcpMixed.Checked Then sec.Mode = 2
        If rbUdp.Checked Then sec.Mode = 3
   

        'uart
        sec.Baudrate = cboBaud.SelectedIndex
        sec.DataBit = cboDataBit.SelectedIndex
        sec.Parity = cboParity.SelectedIndex
        sec.StopBit = cboStopBit.SelectedIndex
        sec.FlowControl = cboFlow.SelectedIndex
        'dns flag & rIP, domain
        If txtRhost.Text.Trim.Length > 0 Then
            If isIpAddr(txtRhost.Text.Trim) Then
                sec.rIp = IpStrToBytes(txtRhost.Text.Trim)
                sec.dnsFlag = 0
            Else
                sec.rDomain = Encoding.ASCII.GetBytes(txtRhost.Text.Trim)
                ReDim Preserve sec.rDomain(31)
                sec.dnsFlag = 1
            End If
        End If
        ReDim sec.rPort(1) : ReDim sec.lPort(1)
        sec.rPort(1) = (CType(txtRport.Text.Trim, UInt16) And &HFF00) >> 8
        sec.rPort(0) = CType(txtRport.Text.Trim, UInt16) Mod 256
        sec.lPort(1) = (CType(txtLport.Text.Trim, UInt16) And &HFF00) >> 8
        sec.lPort(0) = CType(txtLport.Text.Trim, UInt16) Mod 256

        
        'password
        'sec.pwd = Encoding.ASCII.GetBytes(txtPwd.Text.Trim)
        'check if the pwd is changed, 
        ReDim Preserve sec.pwd(15)
        Dim tmp As String = ""
        For i As Integer = 0 To 15
            tmp += gSec.pwd(i).ToString("X2")
        Next
        Debug.Print("tmp: " & tmp)

        txtPwd.Text = txtPwd.Text.Trim
        Debug.Print("txtpwd: " & txtPwd.Text)

        If txtPwd.Text <> tmp Then
            Dim b() As Byte = Encoding.Default.GetBytes(txtPwd.Text.Trim)
            sec.pwd = b
            ReDim Preserve sec.pwd(15)
        ...
        sec.deviceName = Encoding.ASCII.GetBytes(txtDeviceName.Text.Trim)
        ReDim Preserve sec.deviceName(15)

        sec.deviceSerial = gSec.deviceSerial
        ReDim Preserve sec.deviceSerial(15)
    ...
    Private Sub tv_AfterSelect(ByVal sender As System.Object, ByVal e As System.Windows.Forms.TreeViewEventArgs) Handles tv.AfterSelect
        ...
        If lv > 0 Then
            ...
            cboBaud.SelectedIndex = sec.Baudrate
            cboDataBit.SelectedIndex = sec.DataBit
            cboParity.SelectedIndex = sec.Parity
            cboStopBit.SelectedIndex = sec.StopBit
            cboFlow.SelectedIndex = sec.FlowControl
            'mode
            Select Case sec.Mode
                Case 0
                    rbTcpClient.Checked = True
                Case 1
                    rbTcpServer.Checked = True
                Case 2
                    rbTcpMixed.Checked = True
                Case 3
                    rbUdp.Checked = True
            End Select
            txtLport.Text = sec.lPort(0) + sec.lPort(1) * 256
            txtRport.Text = sec.rPort(0) + sec.rPort(1) * 256

            If sec.dnsFlag = 1 Then 'check domain name or ip address
                txtRhost.Text = Encoding.ASCII.GetString(sec.rDomain)
            Else
                txtRhost.Text = BytesToIpStr(sec.rIp)
            End If
           
            'pwd
            'txtPwd.Text = Encoding.ASCII.GetString(sec.pwd)
            Dim tmp As String = ""
            For i As Integer = 0 To 15
                tmp += sec.pwd(i).ToString("X2")
            Next
            txtPwd.Text = tmp
            ...
            'device name & serial number
            txtDeviceName.Text = Encoding.ASCII.GetString(sec.deviceName)
            txtSerialNumber.Text = Encoding.ASCII.GetString(sec.deviceSerial)
...
#End Region
    ...
    Private Sub txtPwd_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtPwd.DoubleClick
